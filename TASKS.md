...
- [x] **Fase 3 — Worker + salida** (2026-06-13): `logic/templates.py` (carga presets, `meta_prompt`, `assemble` con quality_prefix+trigger+prefix/suffix y dedup case-insensitive), `logic/sidecar.py` (naming `<carpeta>_<modelo>_<fmt>`, sanea modelo, copia imágenes, política skip/overwrite/append), `logic/caption_worker.py` (QThread, dual = varias plantillas→varias carpetas, reintentos, progreso, cancelación). Verificado con provider falso: single/dual/skip/dedup OK.
